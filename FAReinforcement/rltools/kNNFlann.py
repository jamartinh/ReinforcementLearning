...
#from scikits.ann2 import kdtree as kdtree2
from pyflann import *
...
class kNNQ(FARL):
    ...
    def __init__(self,nactions,input_ranges,nelemns=[],npoints=0,k=1,alpha=0.3,lm=0.95):
        ...
        #print "tiempo creacion 1",time.clock()-t1
        #t1 = time.clock()
        #self.knntree2 = kdtree2(self.cl)
        #print "tiempo creacion 2",time.clock()-t1
        #self.d2 = add.reduce(self.cl,1)

        self.flann = FLANN()
        self.params = self.flann.build_index(self.cl, target_precision=0.9)
        #print self.params
    ...
    def GetkNNSet(self,s):
        ...
        #FLANN .flatten().flatten()
        self.knn, self.d = self.flann.nn_index(reshape(state,(1,state.shape[0])),self.k, checks=self.params["checks"])
        self.knn = self.knn.flatten()
        self.d   = self.d.flatten()
        self.d   *= self.d
