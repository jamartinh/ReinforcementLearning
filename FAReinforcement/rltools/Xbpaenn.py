...
class NN:
    ...
    def update(self, inputs):
        ...
        self.ah = self.sigmoid(suma)
        # self.ah = sin(suma)
        # self.ah = self.ah / sqrt(sum(self.ah**2))
        # output activations
        suma = matrixmultiply(transpose(self.wo), self.ah)
        self.ao = self.sigmoid(suma)
        # self.ao = sin(suma)
        # self.ao = self.ao / sqrt(sum(self.ao**2))
