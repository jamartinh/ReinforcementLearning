...
class AcrobotEnvironment:
    ...
    def DoAction(self, a, x):
        ...
        for i in range(4):
            ...
            phi1 = -self.m2 * self.l1 * self.lc2 * theta2_dot * sin(theta2) * (theta2_dot - 2 * theta1_dot) + (self.m1 * self.lc1 + self.m2 * self.l1) * self.g * cos(
                    theta1 - (pi / 2.0)) + phi2
    ...
    def InitGraphs(self):
        ...
        self.scene = canvas(x = 0, width = x_width, height = y_height, title = "Acrobot", visible = False)
        ...
        self.r1 = sphere(pos = vector(0, 0, 0), radius = 0.3)
        self.r2 = sphere(pos = vector(x_acrobot[1], y_acrobot[1], 0), radius = 0.3)
        self.r3 = sphere(pos = vector(x_acrobot[2], y_acrobot[2], 0), radius = 0.3)

        self.link1 = curve(pos = [self.r1.pos, self.r2.pos], radius = 0.1)
        self.link2 = curve(pos = [self.r2.pos, self.r3.pos], radius = 0.1)

        self.top = arrow(pos = vector(0, 6, 0), axis = vector(0, -2, 0), shaftwidth = 1)
    ...
    def PlotFunc(self, s, a, steps):
        ...
        self.r2.pos = vector(x_acrobot[1], y_acrobot[1], 0)
        self.r3.pos = vector(x_acrobot[2], y_acrobot[2], 0)

        #self.link1.modify(1, pos = self.r1.pos)
        #self.link1.modify(2, pos = self.r2.pos)
