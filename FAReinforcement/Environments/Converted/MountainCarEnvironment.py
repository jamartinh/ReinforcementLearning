...
#2^n exact numbers for each dimention
    deep_in = [5,5] 
#2^n exact numbers for each dimention
    deep_out= [10]   

    #The number of actions.
    action_list = vector(-1.0 , 0.0 , 1.0)
...
#True
    ...
    def DoAction(self,a,s):
        ...
        if self.graphs:
            if not self.scene.visible:
                ...
                self.clwindow.canvas.visible = True
            self.PlotFunc(force)
    ...
    def initGraphs(self):
        ...
        self.clwindow = graph(x=0, y=0, width=x_width, height=y_height, title="State Space", xtitle='Pos', ytitle='Vel', xmax=0.6, xmin=-1.6, ymax=0.08, ymin=-0.08, foreground=None, background=None)


        self.scene = canvas(x=self.clwindow.width, width=x_width, height=y_height, title="Mountain Car Demo",visible=False)
        ...
        # Init Car
        self.Car = sphere(scene=self.scene,pos=vector(-0.5,sin(3*-0.5)+0.11,0),radius=0.1)

        #Goal
        self.Goal = ring(pos=vector(self.goalPos,sin(3*self.goalPos)+0.1,0),radius=0.2,thickness=0.02)

        # classifiers
        self.clgraph = gdots(graph=self.clwindow)

        #trajectory
        self.Gtrajectory = gcurve(graph=self.clwindow,color=color.yellow)
